...
**Subdirectories (Progressive Disclosure):** Sections support arbitrary directory depth. Subdirectories render as collapsible nodes (collapsed by default). Click to expand and reveal items + nested subdirs. Items have `_subdir` field tracking their full path (e.g., `responses/batch1`). Expansion state persists in localStorage per-notebook via `expandedSubdirs` Set. Key functions: `buildSubdirTree()`, `renderSubdirNode()`, `toggleSubdir()`, `isSubdirExpanded()`.
