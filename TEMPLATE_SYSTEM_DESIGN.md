...
**Decision:** The extension registry defines where file body content goes (e.g., `.md` body → `content` field). Templates must conform to this.
...
├── paper.template.yaml        # Example: user-created template
│
├── theme.css                  # Optional: user CSS overrides
│
└── sections/
    ├── research/
    │   ├── _section.json
    ...
    │   └── reference.bookmark.json
    └── papers/
        ├── _section.json
        └── smith-2024.md      # With template: paper in frontmatter
```
...
card:
  ...
  placeholder: "📄"            # Icon when preview is empty
...
- Add custom extensions (e.g., `.paper.md` → defaults to `paper` template)
...
- ✅ `loadExtensionRegistry()` - Parses `extensions.yaml`, falls back to `getDefaultExtensionRegistry()`
- ✅ `loadTemplates()` - Parses `*.template.yaml` files, falls back to `getDefaultTemplates()`
- ✅ `loadCard()` - Generic card loader using extension registry for parser selection
- ✅ `serializeCard()` - Generic serializer using extension registry
- ✅ `saveCardFile()` - Generic save function handling all card types + companion files
- ✅ `ensureTemplateFiles()` - Creates template files for new notebooks only
- ✅ `renderTemplateButtons()` - Generates "+ Note/Code/Bookmark" buttons from `templateRegistry`
...
|---------|----------|----------|
| How to parse a file | Extension Registry | `.md` → yaml-frontmatter parser |
| Where body content goes | Extension Registry | `.md` body → `content` field |
| Companion file patterns | Extension Registry | `.code.py` → `.output.html` |
| Default template for extension | Extension Registry | `.md` → `note` template |
